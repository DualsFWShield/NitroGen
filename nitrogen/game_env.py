...
class GamepadEnv(Env):
    ...
    def __init__(
        ...
        # Initialize speedhack client and set to normal speed (no slow-motion)
        self.speedhack_client = xsh.Client(process_id=self.game_pid, arch=self.game_arch)
        # MODIFICATION: Set game to run at normal speed and leave it there
        self.speedhack_client.set_speed(1.0)
        print("Xspeedhack disabled - game running at normal speed")
    ...
    def unpause(self):
        ...
        MODIFICATION: No longer used when speedhack is disabled
        """
        pass  # Game always runs at normal speed now
    ...
    def perform_action(self, action, duration):
        """
        Perform the action without pausing/unpausing the game.
        MODIFICATION: Removed pause/unpause calls - game runs continuously
        ...
        # Just wait for the step duration - game runs continuously at normal speed
